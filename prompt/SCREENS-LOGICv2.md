...
### 4.0 Schermata di default vs modalità avanzata

La tab deve mostrare il minimo indispensabile per default, non tutte le funzioni insieme — stesso principio Smart Default già applicato altrove.

**Vista di default (quello che vede la maggior parte degli utenti):**
- Lista di card obiettivo (nome, barra di progresso, importo/scadenza), niente altro visibile
- Un solo pulsante "+" per aggiungere un obiettivo — form minimale (nome, importo, scadenza); la priorità viene assegnata automaticamente in coda al waterfall, non richiesta in fase di creazione
- Tap su una card → dettaglio con storico versamenti
- Card riassuntiva "Finanziamenti" in fondo alla lista, separata dagli obiettivi di risparmio, solo se l'utente ha almeno un mutuo/prestito registrato (altrimenti non compare)

**Modalità avanzata (dietro un'icona impostazioni in alto nella tab, non nel flusso principale):**
- Riordino priorità via drag&drop — questa sezione compare solo se ci sono 2+ obiettivi attivi; con un solo obiettivo non c'è nulla da ordinare, quindi non si mostra
- Switch tra modalità priorità (waterfall) e modalità percentuale (§4.2)
- Condivisione di un obiettivo specifico con il gruppo — accessibile dal dettaglio della singola card (§6.2), non da un'impostazione globale nella lista

Questa separazione vale anche per i Finanziamenti: il calcolatore di fattibilità (§4.4) si apre solo quando l'utente sceglie esplicitamente di aggiungere un mutuo, non è mai in vista di default.
...
## 7. Workflow di calcolo reddito e patrimonio

Principio guida: **la stima da onboarding è sempre sufficiente per iniziare a mostrare numeri**, non blocca nulla — ma deve essere marcata come stima finché non viene confermata, e l'evoluzione da stima a dato certo deve avvenire senza richiedere input ricorrenti forzati all'utente.

### Reddito mensile

- **Open Banking connesso (Pro/Max)**: dopo il collegamento, il sistema analizza lo storico transazioni per rilevare pattern di entrata ricorrente (stessa logica del riconoscimento abbonamenti, applicata alle entrate). Se rilevato con sufficiente confidenza, propone all'utente di sostituire la stima con il valore reale rilevato — un tap per confermare. Se non c'è ancora storico sufficiente, resta valida la stima da onboarding.
- **Senza Open Banking (Free)**: resta la stima dichiarata in onboarding, sempre editabile manualmente. Nessun form periodico forzato — solo un nudge opportunistico quando l'utente registra manualmente un'entrata significativamente diversa dalla stima dichiarata, proponendo di aggiornarla.

### Patrimonio iniziale

- **Open Banking connesso**: il saldo è letto direttamente dai conti collegati, nessun input richiesto.
- **Senza Open Banking**: serve un punto di partenza manuale, ma va offerto come step **opzionale e fuori dalla sequenza principale di onboarding** (per non allungarla) — una card dismissibile al primo accesso in Dashboard ("Vuoi partire dal tuo saldo attuale?"), con opzione esplicita di saltarla e partire da zero, tracciando da quel momento in poi solo le transazioni inserite.

### Stato del dato, non solo il valore

```
user_profile:
  reddito_mensile: numeric
  reddito_stato: enum('stima_onboarding' | 'confermato_manuale' | 'confermato_bancario')
  reddito_aggiornato_il: date
  patrimonio_iniziale: numeric (nullable)
  patrimonio_iniziale_inserito_il: date (nullable)
```

Finché `reddito_stato = 'stima_onboarding'`, la Dashboard mostra l'etichetta "Budget mensile (stima)" con un'icona di modifica sempre accessibile — comunica onestà sul dato senza richiedere nulla in più all'utente finché non è lui a volerlo correggere.
